...
public class SalesProFormaController implements Initializable {
    ...
    private ObservableList<Retailer> retailers= FXCollections.observableArrayList();//= FXCollections.observableArrayList();
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        ObservableList<TableColumn<Parameter<?>,String>> nameColumns=
                FXCollections.observableArrayList(p1ParameterNameColumn,p2ParameterNameColumn,p3ParameterNameColumn, p4ParameterNameColumn, p5ParameterNameColumn);
        ObservableList<TableColumn<Parameter<?>,Object>> totalColumns=
                FXCollections.observableArrayList(p1TotalsColumn,p2TotalsColumn,p3TotalsColumn,p4TotalsColumn,p5TotalsColumn);
        ObservableList<TableColumn<Parameter<?>,Object>> januaryColumns=
                FXCollections.observableArrayList(p1JanuaryColumn,p2JanuaryColumn,p3JanuaryColumn, p4JanuaryColumn, p5JanuaryColumn);
        ObservableList<TableColumn<Parameter<?>,Object>> februaryColumns=
                FXCollections.observableArrayList(p1FebruaryColumn, p2FebruaryColumn,p3FebruaryColumn, p4FebruaryColumn, p5FebruaryColumn);
        ObservableList<TableColumn<Parameter<?>,Object>> marchColumns=
                FXCollections.observableArrayList(p1MarchColumn, p2MarchColumn, p3MarchColumn, p4MarchColumn, p5MarchColumn);
        ObservableList<TableColumn<Parameter<?>,Object>> aprilColumns=
                FXCollections.observableArrayList(p1AprilColumn,p2AprilColumn,p3AprilColumn,p4AprilColumn,p5AprilColumn);
        ObservableList<TableColumn<Parameter<?>,Object>> mayColumns=
                FXCollections.observableArrayList(p1MayColumn, p2MayColumn, p3MayColumn, p4MayColumn, p5MayColumn);
        ObservableList<TableColumn<Parameter<?>,Object>> juneColumns=
                FXCollections.observableArrayList(p1JuneColumn, p2JuneColumn, p3JuneColumn, p4JuneColumn, p5JuneColumn);
        ObservableList<TableColumn<Parameter<?>,Object>> julyColumns=
                FXCollections.observableArrayList(p1JulyColumn,p2JulyColumn,p3JulyColumn,p4JulyColumn,p5JulyColumn);
        ObservableList<TableColumn<Parameter<?>,Object>> augustColumns=
                FXCollections.observableArrayList(p1AugustColumn, p2AugustColumn,p3AugustColumn, p4AugustColumn, p5AugustColumn);
        ObservableList<TableColumn<Parameter<?>,Object>> septemberColumns=
                FXCollections.observableArrayList(p1SeptemberColumn,p2SeptemberColumn,p3SeptemberColumn,p4SeptemberColumn, p5SeptemberColumn);
        ObservableList<TableColumn<Parameter<?>,Object>> octoberColumns=
                FXCollections.observableArrayList(p1OctoberColumn,p2OctoberColumn,p3OctoberColumn,p4OctoberColumn,p5OctoberColumn);
        ObservableList<TableColumn<Parameter<?>,Object>> novemberColumns=
                FXCollections.observableArrayList(p1NovemberColumn,p2NovemberColumn,p3NovemberColumn,p4NovemberColumn,p5NovemberColumn);
        ObservableList<TableColumn<Parameter<?>,Object>> decemberColumns=
                FXCollections.observableArrayList(p1DecemberColumn,p2DecemberColumn,p3DecemberColumn, p4DecemberColumn, p5DecemberColumn);

        ObservableList<ObservableList<TableColumn<Parameter<?>,Object>>> numberColumns =
                FXCollections.observableArrayList(totalColumns,januaryColumns,februaryColumns, marchColumns, aprilColumns,mayColumns,
                        juneColumns,julyColumns, augustColumns, septemberColumns, octoberColumns, novemberColumns, decemberColumns);
...
//                        System.out.println("retailerproduct product " +products.indexOf(product)+ " :"+retailerProduct.getProduct());
...
//                            System.out.println("promoplann2 " +promoPlan);
    ...
    public void updateP1Parameters(int year, int paramIndex) {
        ...
        for (PromoPlan promoPlan : getCurrentPromoPlans(year)) {
            for (int i = 1; i <= 12; i++) {
                switch (paramIndex) {
                    // Part 1
                    case 0:
                        ((BigDecimalParameter)param).setMonth(i, (((BigDecimalParameter)param).getMonth(i)).add((BigDecimal) promoPlan.getParameters().get(0).getMonth(i)));
                        break;
                    case 1:
                        ((IntegerParameter)param).setMonth(i, ((IntegerParameter)param).getMonth(i)+ ((IntegerParameter)promoPlan.getParameters().get(1)).getMonth(i));
                        break;
                    case 2:
                        ((IntegerParameter)param).setMonth(i, ((IntegerParameter)param).getMonth(i)+ ((IntegerParameter)promoPlan.getParameters().get(4)).getMonth(i));
                        break;
                    case 3:
                        ((BigDecimalParameter)param).setMonth(i, ((BigDecimalParameter)param).getMonth(i).add(promoPlan.getSlottingGameTheoryd(i)));
                        break;
                }
            }
        }
        for (int i =1; i <=12; i++) {
            if (paramIndex == 3) {
                ((BigDecimalParameter)param).setTotal(((BigDecimalParameter)param).getTotal().add(((BigDecimalParameter)param).getMonth(i)));

            } else if (paramIndex == 1) {
                System.out.println(paramIndex);
                ((IntegerParameter)param).setTotal(((IntegerParameter)param).getTotal() + ((IntegerParameter)param).getMonth(i));
            }
        }
    }
    public void updateP2Parameters(int year,int paramIndex) {
        ...
        for (PromoPlan promoPlan : getCurrentPromoPlans(year)) {
            for (int i = 1; i <= 12; i++) {
                switch (paramIndex) {
                    case 0:
                        param.setMonth(i, param.getMonth(i).add(promoPlan.getEverydayVolume(i).setScale(4, RoundingMode.HALF_UP)));
                        break;
                    case 1:
    ...
    public void updateP3Parameters(int year, int paramIndex) {
                ...
                switch (paramIndex) {
                    ...
                    case 3:
                        if (promoPlan.getRetailerGrossSales(i).compareTo(new BigDecimal("0.0")) != 0) {
                            param.setMonth(i, param.getMonth(i).add(promoPlan.getRetailerGrossProfit(i).divide(promoPlan.getRetailerGrossSales(i),4,RoundingMode.HALF_UP)));
                        }
                        // Make total function
                        break;
                    case 4:
                        if (promoPlan.getTotalVolume(i).compareTo(new BigDecimal("0.0")) != 0) {
                            param.setMonth(i, param.getMonth(i).add(promoPlan.getRetailerGrossSales(i).divide(promoPlan.getTotalVolume(i),4,RoundingMode.HALF_UP)));
                        }
                        // Make total function
                        break;
                }
        ...
        for (int i =1; i <=12; i++) {
            if (paramIndex != 3 || paramIndex != 4) { // Set totals for bigdecimals
                ...
                param.setMonth(i, param.getMonth(i).divide(new BigDecimal(totalPromoPlansConsidered),4,RoundingMode.HALF_UP));
                param.setTotal(param.getTotal().add(param.getMonth(i)));
            }
    ...
    public void updateP5Parameter(int year) {
        ...
        for (PromoPlan promoPlan : getCurrentPromoPlans(year)) {
            for (int i = 1; i <= 12; i++) {
                if (promoPlan.getTotalVolume(i).compareTo(new BigDecimal("0.0")) > 0) {
                    param.setMonth(i, param.getMonth(i).add(promoPlan.getManufacturerNet1Rev(i).divide(promoPlan.getTotalVolume(1), 4, RoundingMode.HALF_UP)));
                }
            }
        }
        int totalPromoPlansConsidered = getCurrentPromoPlans(year).size();
