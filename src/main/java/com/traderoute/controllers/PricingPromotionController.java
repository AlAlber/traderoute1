...
public class PricingPromotionController implements Initializable, MyController {
            ...
            public void changed(ObservableValue<? extends Retailer> observableValue, Retailer oldRetailer,
                ...
                for (int j = 0; j < 4; j++) {
                    ...
                    oldSkuInDistributionValues
                            .add((BigDecimal) getPromoPlans().get(j).getParameters().get(0).getJanuary());
    ...
    public void changeSkuCountChange(int rowIndex, int colIndex) {
        BigDecimal confidencePer = (BigDecimal) pricingPromotionTableOne.getItems().get(rowIndex + 1)
                .getMonth(colIndex);
        BigDecimal skuCountChange = BigDecimal
                .valueOf((int) pricingPromotionTableOne.getItems().get(rowIndex).getMonth(colIndex));
        ...
        for (int i = colIndex; i <= 12; i++) {
            skusInDistribution.setMonth(i,
                    ((BigDecimalParameter) skusInDistribution).getMonth(i).add(addToSkusInDistribution));
        }
    ...
    public void changeConfidencePer(int rowIndex, int colIndex) {
        BigDecimal skuCountChange = BigDecimal
                .valueOf((int) pricingPromotionTableOne.getItems().get(rowIndex - 1).getMonth(colIndex));
    ...
    public void changePromoRetailRequiredGpm(int colIndex, int promo) {
        ...
        ((BigDecimalParameter) params.get(promoUnitCostIndex)).setMonth(colIndex,
                getCurrentPromoPlan().getPromoCost(colIndex, promo));
        ((BigDecimalParameter) params.get(promoDiscountIndex)).setMonth(colIndex,
    ...
    public void changeMonthCellEvent(TableColumn.CellEditEvent editedCell) {
        ...
        if (rowIndex == everydayRetailIndex) { // ALso needs to depend on GPM, changing , check with john what needs to
                                               // happen with this
            ((BigDecimalParameter) pricingPromotionTableOne.getItems().get(rowIndex + 1)).setMonth(colIndex,
    ...
    public void updateMonthlyTotalVolumeAndGrossProfit() {
        if (showSummaryNotification()) {
            ...
            BigDecimalParameter totalVolumeParam = (BigDecimalParameter) pricingPromotionTableOne.getItems()
            ...
            BigDecimalParameter grossProfitPlanParam = (BigDecimalParameter) pricingPromotionTableOne.getItems()
    ...
    // ObservableList<Parameter<?>> params =pricingPromotionTableOne.getItems();
    // BigDecimal confidencePer = ((BigDecimal) params.get(confidencePerIndex).getMonth(month)).divide(new
    // BigDecimal(100.0));
    ...
    // }
    // return
    // skusInDistribution.multiply(weeklyVelocity).multiply(storeCount).multiply(volumeLiftMultiple).multiply(durationWeeks);
    // }
    //
    ...
    // if (getEverydayCost(month).compareTo(new BigDecimal("0.0"))==0 || getPromoCost(month,promo).compareTo(new
    // BigDecimal("0.0"))==0){
    // return new BigDecimal("0.0");
    // } return new BigDecimal("100").multiply((getEverydayCost(month).subtract(getPromoCost(month,
    // promo))).divide(getEverydayCost(month),4, RoundingMode.HALF_UP));
    // }
    ...
    // return
    // (getEverydayCost(month).multiply(getEverydayVolume(month))).add(getPromoCost(month,1).multiply(getPromoVolume(month,1))).add(getPromoCost(month,2).multiply(getPromoVolume(month,2)));
    // }
    ...
    // return
    // getTotalVolume(month).multiply(getRetailer().getRetailerProducts().get(getRetailer().getCurrentRetailerProductIndex()).getProduct().getUnitListCost());
    // //new BigDecimal("3.59") HARDCODED LIST FOR NOW
    // }
    ...
    // Product product =
    // getRetailer().getRetailerProducts().get(getRetailer().getCurrentRetailerProductIndex()).getProduct();
    ...
    // return (getRetailer().getSpoilsFees()).divide(new
    // BigDecimal("100")).multiply(getManufacturerGrossSalesActual(month));
    // }
    ...
    // return
    // getEverydayAllowanceTS(month).add(getPromoTS(month,1)).add(getPromoTS(month,2)).add(getFixedCostsTS(month)).add(getSpoilsFeesTS(month));
    // }
    ...
    // BigDecimal cogs =
    // getRetailer().getRetailerProducts().get(getRetailer().getCurrentRetailerProductIndex()).getProduct().getUnitBlendedCogs();
    ...
    public void setRetailer(Retailer retailer) {
        ...
        // also update labels in future
        if (getCurrentPromoPlanIndex() > -1) {
            pricingPromotionTableOne
                    .setItems(currentRetailerProduct.getPromoPlans().get(getCurrentPromoPlanIndex()).getParameters());
        }
        for (int i = 0; i < 4; i++) {
