...
public class ProductClassReport {
    ...
    private SimpleObjectProperty<ObservableList<Boolean>> years;
...
//        }
        ...
        System.out.println("total volumetrics 1= "+ totalVolumetrics1);
    ...
    public ObservableList<PromoPlan> getCurrentPromoPlans(){
        ...
        for (int i = 0; i <years.get().size(); i++) {
            Boolean year = years.get().get(i);
            if (isOnlyCommitted() ) {
    ...
    public ObservableList<Boolean> getYears() {
        return years.get();
    }

    public SimpleObjectProperty<ObservableList<Boolean>> yearsProperty() {
        return years;
    }

    public void setYears(ObservableList<Boolean> years) {
