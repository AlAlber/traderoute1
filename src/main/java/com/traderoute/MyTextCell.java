...
public class MyTextCell extends TableCell<Parameter<?>, Object> {
    ...
    public void startEdit() {
        ...
        this.textField.setEditable(param.isEditable());
        TableColumn<Parameter<?>,?> column = getTableColumn();
        int colIndex = getTableView().getColumns().indexOf(column);
        this.textField.setOnAction(evt -> { // enable ENTER commit
            if (param.getJanuary() instanceof String){
                commitEdit(
                        this.textField.getText());
            };
            if (param.getJanuary() instanceof BigDecimal){
                this.textField.setTextFormatter(new TextFormatter(firstTableController.getDoubleInputConverter(), 0.0, firstTableController.getDoubleInputFilter() ));
                commitEdit(
                        new BigDecimal(this.textField.getText()));
            }
        ...
        // Do not allow edit for row where only first item is editable
